<?php
/**
 * @copyright   Copyright (c) 2010-2013 Magify
 */ 
class Magify_Base_Block_Extensions extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        foreach ($modules as $moduleName) {
        	if (strstr($moduleName, 'Magify_') === false) {
        		continue;
        	}
			
			if ($moduleName == 'Magify_Base'){
				continue;
			}
	...
	protected function _getFieldHtml($fieldset, $moduleCode)
    {
		...
		$allExtensions = unserialize(Mage::app()->loadCache('mbase_extensions'));
            
        $status = '<a  target="_blank"><img src="'.$this->getSkinUrl('images/mbase/ok.gif').'" title="'.$this->__("Installed").'"/></a>';
		...
		if ($allExtensions && isset($allExtensions[$moduleCode])){
            ...
            if ($this->_convertVersion($currentVer) < $this->_convertVersion($lastVer)){
                $status = '<a href="'.$url.'" target="_blank"><img src="'.$this->getSkinUrl('images/mbase/update.gif').'" alt="'.$this->__("Update available").'" title="'.$this->__("Update available").'"/></a>';
            }
        }
