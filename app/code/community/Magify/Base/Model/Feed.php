 ...
 */  
class Magify_Base_Model_Feed extends Mage_AdminNotification_Model_Feed
{
    const XML_FREQUENCY_PATH    = 'mbase/feed/check_frequency';
    const XML_LAST_UPDATE_PATH  = 'mbase/feed/last_update';
    const XML_ITERESTS          = 'mbase/feed/interests';
    
    const URL_EXTENSIONS  = 'http://magify.com/feed-extensions.xml';
    const URL_NEWS        = 'http://magify.com/feed-news.xml';

	
	public static function check()
	{
		return Mage::getModel('mbase/feed')->checkUpdate();
	}
    ...
    public function checkUpdate()
    {
        ...
        $wasInstalled = gmdate('Y-m-d H:i:s', Mage::getStoreConfig('mbase/feed/installed'));
        ...
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            ...
            if ($feedData) {
                Mage::app()->saveCache(serialize($feedData), 'mbase_extensions');
            }
        }
    ...
    {
        return Mage::app()->loadCache('mbase_notifications_lastcheck');
    }
 
    public function setLastUpdate()
    {
        Mage::app()->saveCache(time(), 'mbase_notifications_lastcheck');
        return $this;
    }
	...
	protected function isInteresting($item)
	{
		...
		$selfUpgrades = array_search(Magify_Base_Model_Source_Updates_Type::TYPE_INSTALLED_UPDATE, $types);
		...
		foreach ($types as $type){
			...
			if ($extenion && ($type == Magify_Base_Model_Source_Updates_Type::TYPE_UPDATE_RELEASE) && $selfUpgrades){
