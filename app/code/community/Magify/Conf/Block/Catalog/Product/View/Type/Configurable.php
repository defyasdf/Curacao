...
* @package Magify_Conf
*/
class Magify_Conf_Block_Catalog_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Type_Configurable
{
    protected $_optionProducts;
    
    protected function _afterToHtml($html)
    {
        $attributeIdsWithImages = Mage::registry('mconf_images_attrids');
        $list = (Mage::registry('isList')) ? 1 : 0;
        $html = parent::_afterToHtml($html);
        if ('product.info.options.configurable' == $this->getNameInLayout())
        {
            if (Mage::getStoreConfig('mconf/general/hide_dropdowns') || $list)
            {
            ...
            if (Mage::getStoreConfig('mconf/general/show_clear')&& !$list)
            {
                $html = '<a href="#" onclick="javascript: spConfig.clearConfig(); return false;">' . $this->__('Reset Configuration') . '</a>' . $html;
            }
            if (!$list){
                 $html = '<script type="text/javascript">var showAttributeTitle =' . intval(Mage::getStoreConfig('mconf/general/show_attribute_title'))  . '</script>'
                         . '<script type="text/javascript" src="' . Mage::getBaseUrl('js') . 'magify/mconf/configurable.js"></script>'               
                         . $html;
            ...
            {
                $noimgUrl = Mage::helper('mconf')->getNoimgImgUrl();
                ...
                foreach ($simpleProducts as $simple)
                {
                    ...
                    if ($key)
                    {
                        ...
                        if (Mage::getStoreConfig('mconf/general/reload_name'))
                        {
                            $confData[$strKey]['name'] = $simple->getName();
                        }
                        $confData[$strKey]['price'] = $simple->getPrice();
                        if ($simple->getImage() && Mage::getStoreConfig('mconf/general/reload_images'))
                        {
                            $confData[$strKey]['media_url'] = $this->getUrl('mconf/media', array('id' => $simple->getId())); // media_url should only exist if we need to re-load images
                        } elseif ($noimgUrl) 
                        {
                            $confData[$strKey]['noimg_url'] = $noimgUrl;
                        }
                        //for >3
                        if(Mage::getStoreConfig('mconf/general/oneselect_reload')){
                            $pos = strpos($strKey, ",");
                            if($pos){
                ...
                if ($list){
                    $html .= '<script type="text/javascript"> confData['.$this->getProduct()->getEntityId().'] = new AmConfigurableData(' . Zend_Json::encode($confData) . ');
                                                              ...
                                                              confData['.$this->getProduct()->getEntityId().'].mediaUrlMain = "' . $this->getUrl('mconf/media', array('id' => $this->getProduct()->getId())) . '";
                                                              confData['.$this->getProduct()->getEntityId().'].oneAttributeReload = "' . (boolean) Mage::getStoreConfig('mconf/general/oneselect_reload') . '";
                    </script>';
                }
                else {
                    $html .= '<script type="text/javascript"> confData = new AmConfigurableData(' . Zend_Json::encode($confData) . ');
                                                              ...
                                                              confData.mediaUrlMain = "' . $this->getUrl('mconf/media', array('id' => $this->getProduct()->getId())) . '";
                                                              confData.oneAttributeReload = "' . (boolean) Mage::getStoreConfig('mconf/general/oneselect_reload') . '";
                    </script>';
                }
                
                if (Mage::getStoreConfig('mconf/general/hide_dropdowns'))
                {
                    $html .= '<script type="text/javascript">Event.observe(window, \'load\', spConfig.processEmpty);</script>';
                }
                if ('true' == (string)Mage::getConfig()->getNode('modules/Magify_Lbox/active'))
                {
    ...
    protected function getImagesFromProductsAttributes(){
        $collection = Mage::getModel('mconf/product_attribute')->getCollection();
    ...
    public function getJsonConfig()
    {
        ...
        if (Mage::helper('mconf')->getOptionsImageSize()){
             $config['size'] = Mage::helper('mconf')->getOptionsImageSize();
        }
        foreach ($config['attributes'] as $attributeId => $attribute)
        {
            if (Mage::getModel('mconf/attribute')->load($attributeId, 'attribute_id')->getUseImage())
            {
...
//                        
                        $size = Mage::getStoreConfig('mconf/product_image_size/thumb');
                ...
                else if (Mage::getModel('mconf/attribute')->load($attributeId, 'attribute_id')->getUseImage())
                {
                    $config['attributes'][$attributeId]['options'][$i]['image'] = Mage::helper('mconf')->getImageUrl($option['id']);
                }
            }
        }
        Mage::unregister('mconf_images_attrids');
        Mage::register('mconf_images_attrids', $attributeIdsWithImages, true);
