...
* @package Magify_Conf
*/
class Magify_Conf_Helper_Data extends Mage_Core_Helper_Abstract
{
    protected $onClick;
    ...
    public function getImageUrl($optionId)
    {
        ...
        {
            return Mage::getBaseUrl('media') . 'mconf' . '/' . 'images' . '/' . $optionId . '.jpg';
        }
        return '';
    }
    ...
    {
        if (Mage::getStoreConfig('mconf/general/noimage_img'))
        {
            return Mage::getBaseUrl('media') . 'mconf/noimg/' . Mage::getStoreConfig('mconf/general/noimage_img');
        }
        return '';
    }
    ...
    {
            return Mage::getStoreConfig('mconf/general/listimg_size');
    } 
    ...
    public function getHtmlBlock($_product)
    {
        ...
        if($_product->isConfigurable()){
             ...
             $imageBlock = Mage::app()->getLayout()->createBlock('mconf/catalog_product_list_images', 'mconf.catalog_product_list_images');
             $html .= $imageBlock->show($_product->getEntityId());
             $blockForForm = Mage::app()->getLayout()->createBlock('mconf/catalog_product_view_type_configurable', 'mconf.catalog_product_view_type_configurable');
