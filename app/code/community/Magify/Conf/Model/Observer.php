...
* @package Magify_Conf
*/
class Magify_Conf_Model_Observer 
{
    public function onListBlockHtmlBefore($observer)//core_block_abstract_to_html_after    
    {                 
      if (($observer->getBlock() instanceof Mage_Catalog_Block_Product_List) && Mage::getStoreConfig('mconf/general/enable_list')) {
          ...
          foreach ($productsId[1] as $key => $productId){  
              ...
              $blockHtml = Mage::helper('mconf')->getHtmlBlock($_product);
              if($_product->isSaleable()){
                 if($_product->isConfigurable()){
                    ...
                    if($res[0]){
                          $replace =  $res[1][0].$blockHtml.' <div class="actions"><button type="button" title="'.Mage::helper('mconf')->__('Add to Cart').
					'" class="button btn-cart" onclick="'.Mage::helper('mconf')->getClickUrl().
                    '"  mconf="'.Mage::helper('mconf')->getAmconfAttr().'"><span><span>'.Mage::helper('mconf')->__('Add to Cart').'</span></span></button>' ;
        ...
        if (($observer->getBlock() instanceof Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Super_Config)){
            ...
            if (isset($productsId[1])){
                ...
                $controls = '&nbsp;&nbsp;&nbsp;&nbsp;<input onclick=\'return onUseImageProductClick(this);\' type=\'checkbox\' rel="use_image_from_product" id="__id___use_image_from_product" />&nbsp;'.
                        '<label for="__id___use_image_from_product">'. Mage::helper('mconf')->__('Use image from product') .'</label>';
        ...
        if (is_array($configurable_attributes_data)){
            foreach($configurable_attributes_data as $attribute){
                ...
                if ($attribute['id'] !== NULL){
                    $confAttr = Mage::getModel('mconf/product_attribute')->load($attribute['id'], 'product_super_attribute_id');
