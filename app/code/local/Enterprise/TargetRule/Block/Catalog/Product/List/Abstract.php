...
include_once('Enterprise/TargetRule/Block/Catalog/Product/List/Abstract.php');
 
abstract class Enterprise_TargetRule_Block_Catalog_Product_List_Abstract extends Enterprise_TargetRule_Block_Product_Abstract
{
    ...
    protected function _getTargetRuleProducts()
    {
      ...
      // custom
		...
		$cu_ca = $this->getProduct()->getCategoryIds();
		$categoryCollection = Mage::getResourceModel('catalog/category_collection')
							 ->addAttributeToFilter('entity_id', $cu_ca)
							 ->addIsActiveFilter();
		$ca = array();			
        ...
        if($this->_getTypePrefix()=='upsell'){
			$productIds = $prdIds;
		}elseif(count($this->getProduct()->getRelatedProductIds())>0){
			$productIds = $this->getProduct()->getRelatedProductIds();
		}
		...
		if ($productIds) {
			...
			foreach ($productIds as $prods) {
				...
				$_product->load($prods);
				Mage::getSingleton('catalog/product_visibility')->addVisibleInCatalogFilterToCollection($_product);
	            $_product->setPageSize($limit)->setFlag('do_not_use_category_id', true);
				$items[$_product->getEntityId()] = $_product;
            }
        }
