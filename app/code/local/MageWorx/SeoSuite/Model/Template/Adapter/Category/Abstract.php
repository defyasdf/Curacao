...
class MageWorx_SeoSuite_Model_Template_Adapter_Category_Abstract extends MageWorx_SeoSuite_Model_Template_Adapter_Abstract
{
    public function apply($from,$limit,$template,$attribute_name)
    {
        ...
        $select = $connection->select()
                ...
                //->joinLeft(array('seo_store'=>'seosuite_template_store'),'seo_store.template_id=seo_store_template.template_id','')
        ...
        foreach ($catigories as $_category) {
            foreach ($stores as $store) {
                ...
                foreach ($attributes as $attributeId) {
                    $select = $connection->select()->from($tablePrefix.'catalog_category_entity_varchar')->
                                where("entity_type_id = $categoryTypeId AND attribute_id = '$attributeId' AND entity_id = {$category->getId()} AND store_id = {$storeId}");
                    $row = $connection->fetchRow($select);
                    if ($row) {
                    $connection->update($tablePrefix.'catalog_category_entity_varchar', array('value' => $attributeName), "entity_type_id = $categoryTypeId AND attribute_id = '$attributeId' AND entity_id = {$category->getId()} AND store_id = {$storeId}");
                    //echo 'Update '.$tablePrefix."catalog_category_entity_varchar SET value='".$attributeName."' WHERE entity_type_id = $categoryTypeId AND attribute_id = '$attributeId' AND entity_id = {$category->getId()} AND store_id = {$storeId}";
                    } else {
                        $data = array(
                        ...
                        $connection->insert($tablePrefix.'catalog_category_entity_varchar', $data);
                    }
                }
            //    if($category->getId() == 34) exit;
            }
        }
    }
}