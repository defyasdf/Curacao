...
class MageWorx_SeoSuite_Model_Rdfa 
{
        ...
        foreach ($payments as $paymentCode=>$paymentModel) {
            if($paymentModel->canUseCheckout()==1) {
                ...
                if($paymentCode=='ccsave') {
                    $methods[$paymentCode]['cc'] = $this->getCcAvailableTypes($method);
                }
            }
    ...
    private function getPrice()
    {
        ...
        if($product->getTypeInstance(true) instanceof Mage_Bundle_Model_Product_Type) {
            ...
            $html = "<div property='gr:hasMinCurrencyValue' content='".$min."' datatype='xsd:float'></div>";
            $html .= "<div property='gr:hasMaxCurrencyValue' content='".$max."' datatype='xsd:float'></div>";
        }
        else {
            ...
            $html = "<div property='gr:hasCurrencyValue' content='".$price."' datatype='xsd:float'></div>";
        }
        return $html;
    }
    ...
    private function _generate($key,$item)
    {
        ...
        switch ($item['tag']) {
            ...
            case 'typeof':
                list($type,$sKey) = explode(":", $key);
                $content = "about='".Mage::helper("core/url")->getCurrentUrl()."#".$sKey."_".$this->_product->getId()."'";
                break;
