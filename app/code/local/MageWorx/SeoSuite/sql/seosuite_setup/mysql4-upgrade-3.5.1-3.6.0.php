...
foreach (Mage::getModel('core/store')->getCollection()->setLoadDefault(true) as $store) 
{
    ...
    if($templateUrl) {
        $listQuery[] ="('$i', '2', ".$store->getStoreId().", '$templateUrl',NOW())";
        $i++;
    }
    if($templateMetaTitle !=="") {
        $listQuery[] ="('$i', '5', ".$store->getStoreId().", '$templateMetaTitle',NOW())";
        $i++;
    }
    if($templateMetaDescr !=="") {
        $listQuery[] ="('$i', '6', ".$store->getStoreId().", '$templateMetaDescr',NOW())";
    }
}
$string .= join(",\n",$listQuery).";";
