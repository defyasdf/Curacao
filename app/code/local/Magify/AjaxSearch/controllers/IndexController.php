<?php
class Magify_AjaxSearch_IndexController  extends Mage_Core_Controller_Front_Action
{
    private $query = '';
    private $store = '';
    private $enableimage = '';
    ...
    private function _getResults($query, $store)
    {
        ...
        foreach($collection as $product){
            ...
            if($this->enabledescription > 0){
                $description[] =  '<br /><span class="ajaxsearchdescription">'.strip_tags( substr( trim($_product->getShortDescription()),0, $this->descriptionchars)).'</span>';
            } else {
                $description[]='';
            }
        }
        ...
        if (Mage::getStoreConfig('ajax_search/general/enablecatalog')) {
            $categoryCollection = Mage::getResourceModel('ajaxsearch/collection')
                ->getNewCategoryCollection($query, $store);
            $countData[] = count($categoryCollection);

            foreach ($categoryCollection as $category) {
                $suggestion[] = $category['name'];
                $data[] = Mage::getModel("catalog/category")->load($category['entity_id'])->getUrl();
            }
        }
        ...
        if (Mage::getStoreConfig('ajax_search/general/enablecms')) {
            $cmsCollection = Mage::getResourceModel('ajaxsearch/collection')
                ->getNewCmsCollection($query);
            $countData[] = count($cmsCollection);

            foreach ($cmsCollection as $cms) {
                $suggestion[] = $cms['title'];
                $data[] = Mage::getBaseUrl() . $cms['identifier'];
            }
        }
