...
$credit = $proxy->InventoryLevel(array('cItem_ID'=>$products->getSku(),'cLocations'=>'16'));
...
//	$sku = explode("-",$products->getSku());
	
	$sku = str_replace('-','',substr($products->getSku(),0,3));
...
$sq = 'insert into inventory16(product_id, product_sku, storeinv, trashold, ecominv, department) values("'.$products->getId().'","'.$products->getSku().'","'.$storeinv.'","'.$row['thrashold'].'","'.$tot.'", "'.$sku.'")';
mysql_query($sq);
/*
if(sizeof($s)>1){
