...
//  echo $excel->sheets[0]['numCols'];
  ...
  for($i=2;$i<=$excel->sheets[0]['numRows'];$i++){
	 ...
	 if($_GET['type']==1){
		 if($num == 0 && $num1 ==0){ 	
		 $sql = "INSERT INTO `masterproducttable` (`prduct_name`, `product_description`, `product_sku`, `product_upc`, `product_inventory_level`, `product_brand`, `product_img_path`,`product_features`,`product_source`,`product_specs`, product_cost, product_retail, product_msrp, product_map, product_qty, priority) VALUES (";
