<?php 
class Magestore_Storepickup_Model_Order_Pdf_Invoice extends Mage_Sales_Model_Order_Pdf_Invoice
{		
	/**
	...
	**/	
	...
	{	
        ...
        if (!$order->getIsVirtual()) {
			...
			//start change
			$size_image_hight = 625;
			$shiipingInfo = explode("<br/>", $shippingMethod);
			if (count($shiipingInfo)==4){
				$url = strpbrk($shiipingInfo[2], "http");
				$url = explode('/>', $url);				
				$url = rtrim($url[0]);			
				$page->drawText($shiipingInfo[0], 285, $this->y , 'UTF-8');			
				$page->drawText($shiipingInfo[1], 285, $this->y - 10, 'UTF-8');										
			}
			else{
				$url = strpbrk($shiipingInfo[1], "http");
				$url = explode('/>', $url);				
				$url = rtrim($url[0]);			
				$page->drawText($shiipingInfo[0], 285, $this->y , 'UTF-8');		
				$size_image_hight = 635;
			}			
			try{
