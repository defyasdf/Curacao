<?php 
	...
	$query = "SELECT cp.sku,cp.entity_id,cpv.value FROM `catalog_product_entity` as cp, catalog_product_entity_varchar as cpv WHERE cpv.entity_id=cp.entity_id and cpv.attribute_id='183' and cpv.value='2139' and cp.entity_id not in (SELECT cp.entity_id FROM `catalog_product_entity` as cp, catalog_product_entity_varchar as cpv WHERE cpv.entity_id=cp.entity_id and  cpv.value='Domestic' and cpv.attribute_id='179')";
	...
	foreach($product_data as $pdata){
		...
		$credit = $proxy->BatchGetInventory(array('SkuList'=>utf8_encode($skus),'location'=>'16'),
	...
	foreach($qty as $q){
		...
		*/
echo $cquery = "insert into icuracaoproduct.inventorycronstatus(cronday, status) values('".date("Y-m-d H:i:s")."','1')";
