...
(function (window, document, jQuery, undefined) {
	...
	jQuery.extend(F, {
		...
		defaults: {
			...
			tpl: {
				...
				iframe   : '<iframe id="fancybox-frame{rnd}" name="fancybox-frame{rnd}" class="fancybox-iframe" frameborder="0" vspace="0" hspace="0"' + (jQuery.browser.msie ? ' allowtransparency="true"' : '') + '></iframe>',
		...
		_start: function (index) {
			...
			coming.wrap = jQuery(coming.tpl.wrap).addClass('fancybox-' + (isTouch ? 'mobile' : 'desktop') + ' fancybox-type-' + type + ' fancybox-tmp ' + coming.wrapCSS).appendTo( coming.parent );
		...
		_afterLoad: function () {
			...
			current.pos = jQuery.extend({}, current.dim, F._getPosition( true ));
	...
	F.helpers.overlay = {
		...
		onReady: function (opts, obj) {
			...
			if (!this.overlay) {
				jQuery.extend(this, {
					overlay : jQuery('<div class="fancybox-overlay"></div>').appendTo( obj.parent ),
					margin  : D.height() > W.height() || jQuery('body').css('overflow-y') === 'scroll' ? jQuery('body').css('margin-right') : false,
					el : document.all && !document.querySelector ? jQuery('html') : jQuery('body')
				});
			}
		...
		afterClose: function (opts) {
			...
			if (that.overlay && !F.isActive) {
				that.overlay.fadeOut(speed || 0, function () {
					jQuery('body').css('margin-right', that.margin);
