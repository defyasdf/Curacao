...
Product.Config.prototype.fillSelect = function(element){
    ...
    if(options) {
        ...
        if (this.config.attributes[attributeId].use_image)
        {
            ...
            {
                $('mconf-images-' + attributeId).parentNode.removeChild($('mconf-images-' + attributeId));
            }
            ...
            holderDiv.id = 'mconf-images-' + attributeId;
        ...
        for(var i=0;i<options.length;i++){
            ...
            if(allowedProducts.size()>0)
            {
                ...
                if (this.config.attributes[attributeId].use_image)
                {
                    ...
                    imgContainer.addClassName('mconf-image-container');
                    imgContainer.id = 'mconf-images-container-' + attributeId;
                    holderDiv.appendChild(imgContainer);
            
                    var image = document.createElement('img');
                    image = $(image); // fix for IE
                    image.id = 'mconf-image-' + options[i].id;
                    ...
                    if(showAttributeTitle && showAttributeTitle != 0){
                        ...
                        amImgTitle.addClassName('mconf-image-title');
                        amImgTitle.id = 'mconf-images-title-' + options[i].id;
                        amImgTitle.setStyle({
                            fontWeight : 600,
                            textAlign : 'center'
                        });
...
Product.Config.prototype.configureElement = function(element) 
{
    ...
    if ($('mconf-image-' + optionId))
    {
        this.selectImage($('mconf-image-' + optionId));
    } else 
    {
        ...
        $('mconf-images-' + attributeId).childElements().each(function(child){
             if(child.childElements()[0])
                child.childElements()[0].removeClassName('mconf-image-selected');
        });
        }
    }
    // extension Code End
...
Product.Config.prototype.processEmpty = function()
{
    $$('.super-attribute-select').each(function(select) {
        ...
        if (select.disabled)
        {
            ...
            var element = $(select.parentNode).select('#mconf-images-' + attributeId)[0];
