...
Product.Config.prototype.fillSelect = function(element){
    ...
    if(options) {
        ...
        {
            if ($('mconf-images-' + attributeId + '-' + this.config.productId))
            {
                $('mconf-images-' + attributeId + '-' + this.config.productId).parentNode.removeChild($('mconf-images-' + attributeId + '-' + this.config.productId));
            }
            holder = element.parentNode;
            $('label-' + attributeId + '-' + this.config.productId).show();
            var holderDiv = document.createElement('div');
            holderDiv = $(holderDiv); // fix for IE
            holderDiv.addClassName('mconf-images-container');
            holderDiv.id = 'mconf-images-' + attributeId + '-' + this.config.productId;
        ...
        for(var i=0;i<options.length;i++){
            ...
            if(allowedProducts.size()>0)
            {
                ...
                if (this.config.attributes[attributeId].use_image)
                {
                    ...
                    image.id = 'mconf-image-' + options[i].id + '-' + this.config.productId;
		    image.src   = options[i].image;
                    image.width = this.config.size;
                    image.height = this.config.size;
                    image.addClassName('mconf-image');
        ...
        if(index > 1 && this.config.attributes[attributeId].use_image) {
            ...
            amcart.id = 'mconf-amcart-' + this.config.productId;
...
Product.Config.prototype.configureElement = function(element) 
{
    ...
    if ($('mconf-image-' + optionId + '-' + this.config.productId))
    {
        this.selectImage($('mconf-image-' + optionId + '-' + this.config.productId));
    } 
    else {
        attributeId = element.id.replace(/[a-z-]*/, '');
        if ($('mconf-images-' + attributeId))
        {
            $('mconf-images-' + attributeId).childElements().each(function(child){
                child.removeClassName('mconf-image-selected');
            });
        }
    }
...
Product.Config.prototype.selectImage = function(element)
{
    ...
    element.addClassName('mconf-image-selected');
    ...
    if ('-1' != pos) {
          ...
          if(confData[parentId]['optionProducts'][key]){ 
             $$('.product-image img').each(function(img){
                  if(img.src == confData[parentId]['optionProducts'][key]['parent_image'] || img.className == ('mconf-parent-'+parentId)){
                      img.src = confData[parentId]['optionProducts'][key]['small_image'];
                      img.addClassName('mconf-parent-'+parentId);
                  }
             });              
          }
    }  
