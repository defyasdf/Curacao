...
jQuery(function($) {
	...
	function _qsJnit(){
		...
		if(Magify.QuickShop.BASE_URL.indexOf('index.php') == -1){
			mypath = 'index.php/quickshop/index/view';
		}
		var baseUrl = Magify.QuickShop.BASE_URL + mypath;
		
		var _qsHref = "<a id=\"magify_quickshop_handler\" href=\"#\" style=\"visibility:hidden;position:absolute;top:0;left:0\"><img  alt=\"quickshop\" src=\""+Magify.QuickShop.QS_IMG+"\" /></a>";
		$(document.body).append(_qsHref);
		
		var qsHandlerImg = $('#magify_quickshop_handler img');
		...
		$.each(listprod, function(index, value) { 
			...
			$(selectorObj.imgClass, this).bind('mouseover', function() {
				var o = $(this).offset();
				$('#magify_quickshop_handler').attr('href',reloadurl).show()
					.css({
			...
			$(value).bind('mouseout', function() {
				$('#magify_quickshop_handler').hide();
			});
		});

		//fix bug image disapper when hover
		$('#magify_quickshop_handler')
		...
		$('#magify_quickshop_handler').fancybox({
				'width'				: Magify.QuickShop.QS_FRM_WIDTH,
				'height'			: Magify.QuickShop.QS_FRM_HEIGHT,
				'autoScale'			: false,
				'padding'			: 0,
				'margin'			: 0,
				//'transitionIn'		: 'none',
