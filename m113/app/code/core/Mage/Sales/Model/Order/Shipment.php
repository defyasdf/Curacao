...
class Mage_Sales_Model_Order_Shipment extends Mage_Sales_Model_Abstract
{
    ...
    public function register()
    {
        ...
        foreach ($this->getAllItems() as $item) {
			...
			if($item->getQty()>0){
				$this->setarsupply($this->getOrderId(),$item->getSku(),$item->getQty());
			}
			//echo $this->getOrderId()." : ".$item->getSku().' : '.$item->getQty();
        }
        $this->setTotalQty($totalQty);
		//exit;
        return $this;
    }
	public function setarsupply($order_id,$sku,$qty){
		
		$order = Mage::getModel('sales/order')->load($order_id);
		
		if($order->getEstimatenumber()){		
			...
			$credit = $proxy->SupplyWebItem(array('InvNo'=>$order->getEstimatenumber(),'ItemID'=>$sku,'Qty'=>$qty));
			$result = $credit->SupplyWebItemResult;
			...
			$order->addStatusToHistory($order->getStatus(), 'There is no estimate associated for this order', false);
			$order->save();
		}
